...
public class Squares_With_Three_Sides {
    ...
    private static int getVerticalTriangleAmount(String input) {
        ...
        for (int i = 0; i<triangles.size()/3; i++) {
            int[][] threeSides = rearangeLines(getTriangleSides(triangles.get(i*3)), getTriangleSides(triangles.get((i*3)+1)), getTriangleSides(triangles.get((i*3)+2)));
            for (int j = 0; j<3; j++) {
                int[] sides = threeSides[j];
                if (checkForValidTriangle(sides)) counter++;
            }
        }
