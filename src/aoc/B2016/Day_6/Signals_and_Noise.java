...
public class Signals_and_Noise {
    ...
    private static char getLeastCommonCharOfString(String input) {
        ...
        for (int i = 0; i < input.length(); i++) {
            if (characterAmount.containsKey(input.charAt(i))) characterAmount.replace(input.charAt(i), characterAmount.get(input.charAt(i))+1);
            else characterAmount.put(input.charAt(i), 1);
        }
        char leastUsedChar = ' ';
